...
from loader import dp, bot, ADMIN_ID, ADMIN_IDS
...
class VacancyForm(StatesGroup):
    ...
    ref_consent      = State()   # 14
    ref_person       = State()   # 15
    work_duration    = State()   # 16
    overtime         = State()   # 17
    health_ok        = State()   # 18
    late_reason      = State()   # 19
    theft_reason     = State()   # 20
    perf_reason      = State()   # 21
    prev_salary      = State()   # 22
    expected_salary  = State()   # 23
    courses          = State()   # 24
    confirm          = State()
...
async def bot_start(message: types.Message, state: FSMContext):
    await state.finish()
    _save_user_id(message.from_user.id)
    await message.answer(
            ...
            for admin_id in ADMIN_IDS:
                try:
                    await bot.send_voice(
                        chat_id=admin_id,
                        voice=voice_id,
                        caption="🎙 10-savol: Oila haqida ovozli xabar"
                    )
                except Exception:
                    pass
...
async def show_preview_sotuvchi(message: types.Message, state: FSMContext):
    ...
    preview = (
        ...
        f"13. <b>Surishtirishga rozilik:</b> {data.get('ref_consent', '—')}\n"
...
async def sotuvchi_get_ref_consent(message: types.Message, state: FSMContext):
    ...
    if message.text == "✅ Ha":
        ...
        await message.answer(
            "📝 <b>13/22</b> — Siz haqingizda ma'lumot bera oladigan kishi bormi?\n"
            "<i>Ismi, aloqasi va siz uni qayerdan tanishingizni yozing</i>",
            parse_mode="HTML", reply_markup=types.ReplyKeyboardRemove()
        )
    else:
...
async def sotuvchi_confirm_form(message: types.Message, state: FSMContext):
    ...
    if message.text == "✅ Ha, yuborish":
        ...
        caption = (
            ...
            f"12. <b>Surishtirishga rozilik:</b> {data.get('ref_consent', '—')}\n"
